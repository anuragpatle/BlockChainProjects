...
const Badge = ({ text, color }) => (
  <div className="flex-initial w-40">
    <span
      className={`bg-${color}-100 text-${color}-800 text-sm font-medium mr-2 px-2.5 py-0.5 rounded dark:bg-${color}-700 dark:text-${color}-300`}
    >
      {text}
    </span>
    {/* <span
  ...
  <div className="flex flex-row justify-start items-start white-glassmorphism p-3 m-2 cursor-pointer hover:shadow-xl">
    ...
    <div className="ml-5 flex flex-col flex-1">
      ...
      <div className="flex mt-2">
        <Badge text="Drug" color="gray"></Badge>
        <div className=" mt-1 text-white text-sm md:w-9/12">{drugName}</div>
      </div>

      <div className="flex mt-2">
        <Badge text="Vendor" color="green"></Badge>
      ...
      <div className="grid sm:grid-cols-2 grid-cols-2 w-full mt-10">
        <button
          type="button"
          className="bg-[#2e6242] bg-green-100 text-white font-bold w-32 mt-2 border-[1px] p-2 border-[#3d4f7c] hover:bg-[#3d4f7f] rounded-full cursor-pointer"
        >
          Yes
        </button>
        <button
...
const ConfirmDelivery = () => {
  const { dispatchOrdersArray } = useContext(TransactionContext);
        ...
        <div className="flex mf:flex-row flex-col items-center justify-between md:p-5">
          <div className="flex-1 flex flex-col justify-start items-center">
            {/* <div className="grid sm:grid-cols-3 grid-cols-2 w-full mt-10"> */}
              {console.log("dispatchOrdersArray: : : ", dispatchOrdersArray)}
              {dispatchOrdersArray.map((order) => (
                ...
                <ServiceCard
                  ...
                  subtitle={`${order.temperature} °C as permissible Temperature`}
